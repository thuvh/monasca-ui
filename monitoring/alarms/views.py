...
class AlarmHistoryView(tables.DataTableView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if ts_mode and ts_mode != alarm_history_default_ts_format:
            ts_offset_label = (
                [a for a in alarm_history_ts_formats if a[0] == ts_mode][0][1])
            ts_changed_msg = _('Timestamp format set to %s [%s]'
                               % (ts_offset_label, ts_offset or '0'))
            messages.success(self.request, ts_changed_msg)
