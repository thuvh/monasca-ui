...
class GraphMetricLinkActionTests(TestCase):
    ...
    def test_allowed_grafana_liniks(self):
        allowed = self.get_allowed()
        self.assertEqual(allowed, False)

    @override_settings(GRAFANA_LINKS=[])
    def test_allowed_grafana_links_empty(self):
        allowed = self.get_allowed()
        self.assertEqual(allowed, False)

    @override_settings(GRAFANA_LINKS=[{'title': 'Dashboard', 'path': '/grafana/test'}])
