    ...
    def get(self, request, *args, **kwargs):
        ...
        if "metrics" == parts[0]:
            ...
            if "statistics" == parts[1]:
                results = {'elements': api.monitor.metrics_stat_list(request,
                                                   **req_kwargs)}
            if "measurements" == parts[1]:
                results = {'elements': api.monitor.metrics_list(request,
                                                   **req_kwargs)}
        if not results:
            LOG.warn("There was a request made for the path %s that is not supported." % restpath)
            results = {}
