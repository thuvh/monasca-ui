...
function run_pep8 {
  echo "Running flake8 ..."
  DJANGO_SETTINGS_MODULE=monitoring.test.settings ${command_wrapper} flake8 $included_dirs
}
...
function run_tests_all {
  echo "Running Monasca-UI application tests"
  export NOSE_XUNIT_FILE=monitoring/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='monasca_ui_nose_results.html'
  fi
  ${command_wrapper} ${COVERAGE_CMD} erase
  ${command_wrapper} ${COVERAGE_CMD} run -p $root/manage.py test monitoring --settings=monitoring.test.settings $testopts
  ...
  if [ $with_coverage -eq 1 ]; then
    ...
    ${command_wrapper} ${COVERAGE_CMD} xml -i --omit='monitoring/test/*,**/shiningpanda/**,/usr*,setup.py,*egg*,.venv/*'
    ${command_wrapper} ${COVERAGE_CMD} html -i --omit='monitoring/test/*,**/shiningpanda/**,/usr*,setup.py,*egg*,.venv/*' -d reports
  fi
  ...
  TEST_RESULT=$(($MONASCA_UI_RESULT || $PEP8_RESULT))
